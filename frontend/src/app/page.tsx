...
import { Plus, Edit3, Save, X, Trash2, MapPin, RefreshCw } from 'lucide-react';
...
export default function HomePage() {
  ...
  const fetchRecentBuildings = useCallback(async () => {
    try {
      ...
      if (data && Array.isArray(data)) {
        const pins = data.map((building: Building) => ({
          id: building.id,
  ...
  const startEditing = () => {
    if (selectedPin) {
      setEditingBuilding({
        id: selectedPin.id,
        lat: selectedPin.position[0],
        long: selectedPin.position[1],
        address: selectedPin.title,
        doors: selectedPin.doors || [],
        language: selectedPin.language || 'English',
      });
      setIsEditing(true);
    }
  };
        ...
        setRecentPins(prevPins => 
          prevPins.map(pin => 
            pin.id === editingBuilding.id 
              ? {
                  ...pin,
                  position: [editingBuilding.lat, editingBuilding.long],
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setRecentPins(prevPins => prevPins.filter(pin => pin.id !== id));
      setSelectedPin(null);
